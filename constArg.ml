...
let rec setConstArgs env = function
  ...
  | LetRec({ name = (x, _); args = ys; body = e1 }, e2) ->
      constArgs := M.add x (List.map (fun _ -> (0, Unit)) ys) !constArgs;
      setConstArgs env e1;
      setConstArgs env e2
  | App(x, ys) ->
...
let rec g = function
  ...
  | LetRec({ name = (x, Type.Fun(ts, t)); args = ys; body = e1 }, e2) ->
      let (ys', e1') = List.fold_right2
                 (fun y (n, v) (ys, e) ->
                    if n = 1 then (Format.eprintf "Const: %s->%s=%s@." x (fst y) (str v); (ys, Let(y, v, e)))
                    else (y :: ys, e))
  ...
  | App(x, ys) ->
      App(x, List.fold_right2 (fun y (n, _) ys -> if n = 1 then ys else y :: ys) ys (M.find x !constArgs) [])
