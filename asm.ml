...
let regs = Array.init 11 (fun i -> Printf.sprintf "$i%d" (i + 1))
let fregs = Array.init 14 (fun i -> Printf.sprintf "$f%d" (i + 1))
...
let reg_asm = "$i12" (* for tmp *)
let reg_fasm = "$f15" (* for tmp *)
let reg_zero = "$zero" (* 0 *)
let reg_fzero = "$fzero" (* 0.0 *)
let is_reg x = (x.[0] = '$')
