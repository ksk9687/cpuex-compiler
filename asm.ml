...
let regs = Array.init 12 (fun i -> Printf.sprintf "$%d" (i + 2))
let fregs = Array.init 13 (fun i -> Printf.sprintf "$%d" (i + 16))
...
let reg_sp = "$0" (* stack pointer *)
let reg_hp = "$1" (* heap pointer (caml2html: sparcasm_reghp) *)
let reg_ra = "$31" (* return address *)
let reg_asm = "$15" (* for assembler *)
let reg_fasm = "$30" (* for assembler *)
let reg_zero = "$14" (* 0 *)
let reg_fzero = "$29" (* 0.0 *)
let is_reg x = (x.[0] = '$')
