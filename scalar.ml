...
let rec g saved (dest, e) =
  ...
  let e1, saved' =
    if (not saved) && hasNonTailCall tail e then
      let e1 = g' saved (NonTail(reg_sp), Sub(reg_sp, C(!stacksize))) in
      let e2 = g' saved (NonTail(reg_tmp), St(reg_ra, V(reg_sp), C(0))) in
      (seq e1 e2, true)
    else if saved && tail && notHasNonTailCall tail e then
      let e1 = g' saved (NonTail(reg_ra), Ld(V(reg_sp), C(0))) in
      let e2 = g' saved (NonTail(reg_sp), Add(reg_sp, C(!stacksize))) in
      (seq e1 e2, false)
    else
      (End, saved)
  in
...
and g' saved = function
  | NonTail(_), Nop -> End
  | NonTail(x), Set(i) when i < 0 -> g' saved (NonTail(x), Add(reg_zero, C(i)))
  | NonTail(x), Set(i) -> Seq(Exp(Printf.sprintf "\t%-8s%d, %s\n" "li" i x, [], [x]), End)
  | NonTail(x), SetL(Id.L(y)) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "li" y x, [], [x]), End)
  | NonTail(x), Mov(y) when x = y -> End
  | NonTail(x), Mov(y) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "mov" y x, [y], [x]), End)
  | NonTail(x), Neg(y) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "neg" y x, [y], [x]), End)
  | NonTail(x), Add(y, z') -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s, %s\n" "add" y (pp_id_or_imm z') x, y :: (reg z'), [x]), End)
  | NonTail(x), Sub(y, z') -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s, %s\n" "sub" y (pp_id_or_imm z') x, y :: (reg z'), [x]), End)
  | NonTail(x), SLL(y, i) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %d, %s\n" "sll" y i x, [y], [x]), End)
  | NonTail(x), Ld(y', C(z)) when z < 0 -> Seq(Exp(Printf.sprintf "\t%-8s[%s - %d], %s\n" "load" (pp_id_or_imm y') (-z) x, "memory" :: (reg y'), [x]), End)
  | NonTail(x), Ld(y', z') -> Seq(Exp(Printf.sprintf "\t%-8s[%s + %s], %s\n" "load" (pp_id_or_imm y') (pp_id_or_imm z') x, "memory" :: (reg y')@(reg z'), [x]), End)
  | NonTail(r), St(x, V(y), V(z)) ->
      seq (g' saved (NonTail(reg_tmp), Add(y, V(z)))) (g' saved (NonTail(r), St(x, V(reg_tmp), C(0))))
  | NonTail(_), St(x, y', C(z)) when z < 0 -> Seq(Exp(Printf.sprintf "\t%-8s%s, [%s - %d]\n" "store" x (pp_id_or_imm y') (-z), x :: (reg y'), ["memory"]), End)
  | NonTail(_), St(x, y', z') -> Seq(Exp(Printf.sprintf "\t%-8s%s, [%s + %s]\n" "store" x (pp_id_or_imm y') (pp_id_or_imm z'), x :: (reg y')@(reg z'), ["memory"]), End)
  | NonTail(x), FNeg(y) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "fneg" y x, [y], [x]), End)
  | NonTail(x), FInv(y) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "finv" y x, [y], [x]), End)
  | NonTail(x), FSqrt(y) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "fsqrt" y x, [y], [x]), End)
  | NonTail(x), FAbs(y) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s\n" "fabs" y x, [y], [x]), End)
  | NonTail(x), FAdd(y, z) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s, %s\n" "fadd" y z x, [y; z], [x]), End)
  | NonTail(x), FSub(y, z) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s, %s\n" "fsub" y z x, [y; z], [x]), End)
  | NonTail(x), FMul(y, z) -> Seq(Exp(Printf.sprintf "\t%-8s%s, %s, %s\n" "fmul" y z x, [y; z], [x]), End)
  | NonTail(x), LdFL(Id.L(l)) -> Seq(Exp(Printf.sprintf "\t%-8s[%s], %s\n" "load" l x, [], [x]), End)
  | NonTail(_), MovR(x, y) -> g' saved (NonTail(y), Mov(x))
  | NonTail(r), Save(x, y) when List.mem x allregs && not (S.mem y !stackset) ->
      save y;
      g' saved (NonTail(r), St(x, V(reg_sp), C(offset y - (if saved then 0 else !stacksize))))
  | NonTail(_), Save(x, y) -> assert (S.mem y !stackset); End
  | NonTail(x), Restore(y) ->
      assert (List.mem x allregs);
      g' saved (NonTail(x), Ld(V(reg_sp), C(offset y - (if saved then 0 else !stacksize))))
  | Tail, (Nop | St _ | MovR _ | Save _ as exp) ->
      seq (g' saved (NonTail(Id.gentmp Type.Unit), exp)) ret
  | Tail, (Set _ | SetL _ | Mov _ | Neg _ | Add _ | Sub _ | SLL _ | Ld _ |
           FNeg _ | FInv _ | FSqrt _ | FAbs _ | FAdd _ | FSub _ | FMul _ | LdFL _ as exp) ->
      seq (g' saved (NonTail(regs.(0)), exp)) ret
  ...
  | Tail, CallCls(x, ys) ->
      ...
      let e = seq e (g' saved (NonTail(reg_tmp), Ld(V(reg_cl), C(0)))) in
      seq e (Seq(Exp(Printf.sprintf "\t%-8s%s\n" "jr" reg_tmp, [], ["all"]), End))
  ...
  | NonTail(a), CallCls(x, ys) ->
      ...
      let e = seq e (Seq(Exp(Printf.sprintf "\t%-8s%s\n" "jr" reg_tmp, [], ["all"; "cond"]), End)) in
      let e = seq e (Seq(Exp(Printf.sprintf "%s:\n" ra, [], ["all"]), End)) in
      if List.mem a allregs && a <> regs.(0) then seq e (g' saved (NonTail(a), Mov(regs.(0))))
      else e
  | NonTail(a), CallDir(Id.L(x), ys) ->
      let e = g'_args [] ys in
      let e = seq e (Seq(Exp(Printf.sprintf "\t%-8s%s\n" "jal" x, [], ["all"; "cond"]), End)) in
      if List.mem a allregs && a <> regs.(0) then seq e (g' saved (NonTail(a), Mov(regs.(0))))
      else e
...
and g'_args x_reg_cl ys =
  let (i, yrs) =
    List.fold_left
      ...
      ys in
  List.fold_right
    (fun (y, r) e -> seq (g' true (NonTail(r), Mov(y))) e)
