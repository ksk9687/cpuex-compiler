...
let rec g env = function (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | Add(x, y) when memi x env && M.find x env = Int(0) -> g env (Var (y))
  | Add(x, y) when memi y env && M.find y env = Int(0) -> g env (Var (x))
  ...
  | FSub(x, y) when memf x env && M.find x env = Float (0.) -> g env (FNeg (y))
