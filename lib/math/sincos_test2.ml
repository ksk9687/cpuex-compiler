...
(* 焼け石に水だが、多少は reduction の精度が良いはず・・・ *)
...
let rec sin x =
  ...
  in
  let rec sin_sub x pi2 =
      if pi2 > x then x
      else
        let x2 = sin_sub x (pi2 *. 2.) in
          if x2 < pi2 then x2
          else x2 -. pi2
  in
...
(* let gen2 () = (exp (Random.float 80.0)) in *)
let gen2 () = (pi +. (Random.float 0.0001)) in
let rec check f n = 
  let rec sub n ex = 
    if n == 0 then ex
    else
